...
num_ips=`wc -l /tmp/access_ip.log` # Contamos el número de líneas que hay en el fichero de IPs, ése es el número que nos piden
num_404=`wc -l /tmp/access_404.log` # Contamos el número de líneas también en el fichero de respuestas con 404. También es lo que nos piden.
...
accesos_mes=`grep -Eo "$mes\/\d+:\d+:\d+:\d+\s\S+\]" $1 | wc -l | sed -e 's/^[[:space:]]*//'` # El número de accesos en Enero. Lo obtenemos usando una expresión regular, contamos el número de líneas que coinciden, y el sed elimina los espacios que aparecen al principio del número. En este caso no hace falta que usemos awk para eliminar el nombre del archivo, porque cuando en wc se lee desde stdin no se indica nada más que el número.
...
accesos_mes=`grep -Eo "$mes\/\d+:\d+:\d+:\d+\s\S+\]" $1 | wc -l | sed -e 's/^[[:space:]]*//'`
