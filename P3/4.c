...
sem_t *empty, *mutex, *full; // Variables semaforo
...
{
    pthread_t threads_productores[NUM_PRODUCTORES];
    ...
    // Mapeamos la memoria compartida. Ya no hace falta realmente, pero lo mantengo por coherencia con los apartados anteriores.
    buffer = (int *)mmap(NULL, sizeof(int) * TAM_BUFFER, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    cuenta = (int *)mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    ...
    *cuenta = 0; // Inicializamos la variable cuenta a 0. Deberia venir ya inicializada, pero creo que es buena practica asegurarnos.
    ...
    {
        num_elementos_productores[i] = (i == 0 ? ((NUM_ELEMENTOS_TOTALES / NUM_PRODUCTORES) + (NUM_ELEMENTOS_TOTALES % NUM_PRODUCTORES)) : (NUM_ELEMENTOS_TOTALES / NUM_PRODUCTORES));
    ...
    {
        num_elementos_consumidores[i] = (i == 0 ? ((NUM_ELEMENTOS_TOTALES / NUM_CONSUMIDORES) + (NUM_ELEMENTOS_TOTALES % NUM_CONSUMIDORES)) : (NUM_ELEMENTOS_TOTALES / NUM_CONSUMIDORES));
